...
fn test_descriptor_rejects_an_alias_that_duplicates_its_canonical_id() {
    ...
    assert_eq!("provider alias matches canonical ID: file-command", error.to_string());
...
fn test_descriptor_reports_invalid_alias_with_position_and_source() {
    ...
    assert_eq!("invalid provider alias at index 1: \"bad alias\"", error.to_string());
...
fn test_descriptor_distinguishes_duplicate_aliases() {
    ...
    assert!(matches!(error, ProviderDescriptorError::DuplicateAlias { .. }));
}
