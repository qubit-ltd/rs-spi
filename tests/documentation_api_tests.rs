...
fn test_provider_input_fuzz_target_bounds_input_before_parsing() {
    ...
    let bound = target.find("const MAX_INPUT_BYTES: usize = 4096;").expect(
        "provider-input fuzz target must declare the CI-aligned byte bound",
    );
    let guard = target
        .find("if data.len() > MAX_INPUT_BYTES {")
        .expect("provider-input fuzz target must reject oversized inputs");
    let parse = target.find("str::from_utf8(data)").expect(
        "provider-input fuzz target must parse its byte input as UTF-8",
    );

    assert!(
        bound < guard,
        "the byte bound must be declared before its guard"
    );
    assert!(
        guard < parse,
        "the byte guard must run before UTF-8 parsing"
    );
}
