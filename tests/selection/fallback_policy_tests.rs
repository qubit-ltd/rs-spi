...
fn test_selection_uses_on_absence_by_default() {
    assert_eq!(FallbackPolicy::OnAbsence, ProviderSelection::auto().fallback_policy());
}
...
fn test_fallback_policy_continues_after_expected_failure_kinds() {
    for (policy, kind, expected) in [
        ...
        (FallbackPolicy::Never, ProviderFailureKind::InvalidConfiguration, false),
        (FallbackPolicy::Never, ProviderFailureKind::InitializationFailed, false),
        (FallbackPolicy::OnAbsence, ProviderFailureKind::Unsupported, true),
        (FallbackPolicy::OnAbsence, ProviderFailureKind::Unavailable, true),
        (
        ...
        (FallbackPolicy::OnAnyError, ProviderFailureKind::Unsupported, true),
        (FallbackPolicy::OnAnyError, ProviderFailureKind::Unavailable, true),
        (
