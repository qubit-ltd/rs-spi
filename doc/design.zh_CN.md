...
本文说明 0.13 API 的架构与实现契约，面向维护者和领域 crate 作者。可运行的集成
...
没有注销或解除封存操作。应用完成启动注册和默认选择配置后可以调用 `seal()`；
之后的注册表变更都会被拒绝。没有具体领域需求时，不增加成功输出元数据包装、
依赖注入容器或动态插件加载器。继续共用目录和失败状态机，把领域专用校验留在适配层。未来若
